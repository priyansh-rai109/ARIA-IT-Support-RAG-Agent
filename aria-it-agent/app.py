...
def retrieve_context(query, k=3):
    if not vectorstore:
        return [], ""
    try:
        # Chroma default HNWS uses squared L2 distance. Smaller scores represent higher similarity.
        results = vectorstore.similarity_search_with_score(query, k=k)
        filtered = []
        for doc, dist in results:
            # Map L2 distance to confidence score where 0 distance is 1.0 confidence
            confidence = max(0.0, min(1.0, 1.0 - (dist / 2.0)))
            if confidence > 0.35:
                filtered.append((doc, confidence))
        
        context_parts = [f"[{doc.metadata['title']}]\n{doc.page_content}" for doc, conf in filtered]
        return filtered, "\n\n".join(context_parts)
    except Exception as e:
...
.suggested-actions * { background: transparent !important; color: white !important; }
...
.action-subtext { font-size: 11px; color: var(--muted) !important; }
.action-arrow { margin-left: auto; color: var(--muted) !important; }
...
def build_ui():
    with gr.Blocks(css=CSS, title="ARIA IT Support") as demo:
        ...
        with gr.Row(elem_classes="main-layout"):
            ...
            with gr.Column(scale=30, min_width=300, elem_classes="right-col"):
                ...
                if robot_b64:
                    ...
                    gr.HTML('<div class="robot-img" style="height: 300px; display:flex; align-items:center; justify-content:center; background:rgba(255,255,255,0.02); border:1px dashed var(--border); border-radius:16px; color:var(--muted);">🤖 ARIA MASCOT</div>')
                gr.HTML("""
        ...
        for btn, query in [
            ...
            (btn_jira, "Create a new Jira ticket for the active IT request."),
            (btn_infra, "Page the infrastructure team about the on-call alert."),
            (btn_email, "Draft an email response to the user regarding their IT support issue."),
            (btn_security, "Initiate a high-priority security escalation.")
        ]:
